...
Hooks.on('renderNoteConfig', async (app, html, noteData) => {
  ...
  const numberWsSuffixOnNameplate = (app.document
  ...
  const usePinIsRevealed = foundry.utils.getProperty(noteData, `document.flags.${CONSTANTS.MODULE_ID}.${CONSTANTS.FLAGS.USE_PIN_REVEALED}`)
    ?? false;
  ...
  const previewAsTextSnippet = (app.document
  ...
  const tooltipForceRemove = (app.document
  ...
  const tooltipSmartPlacement = (app.document
  ...
  const tooltipFollowMouse = (app.document
  ...
  const tooltipCustomDescription = (app.document
